...
Next.js App Router + React 19 + Tailwind v4, drag-and-drop via `@dnd-kit`. `lib/api.ts` is the single typed client for every backend call (auth, boards, per-board AI chat) and throws `ApiError` carrying the HTTP status. `components/AppShell.tsx` is the top-level controller: gates on auth (login/register), loads the user's boards via `listBoards`, tracks the active board (`activeBoardIdRef` + `boardVersionRef` for synchronous version-checked saves), and hosts the AI chat panel pointed at the active board. `components/BoardSwitcher.tsx` renders the board tabs (select/create/rename/delete, guarding the last board; rename/delete hidden on shared `editor` boards) in the `KanbanBoard` header `toolbar` slot, alongside a Share button. `components/ShareDialog.tsx` lists members and lets the owner add/remove them. `components/CardEditor.tsx` edits a card's title/details/labels/due date. `KanbanBoard`/`KanbanColumn`/`KanbanCard` are presentation controlled by AppShell state; `KanbanBoard` owns card search + the active card editor. `lib/kanban.ts` holds board types and `moveCard`; `lib/aiChat.ts` holds chat history helpers. Full component map: `frontend/AGENTS.md`.
